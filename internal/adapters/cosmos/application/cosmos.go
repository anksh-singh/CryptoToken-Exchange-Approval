...
func NewCosmosServerHandler(config *config.Config, log *zap.SugaredLogger) (*CosmosServerHandler, error) {
	httRequest := utils.NewHttpRequest(log)
	utilsManager := utils.NewUtils(log, config)
	cosmosHandler, err := core.NewCosmosHandler(config, log, httRequest, utilsManager)
	if err != nil {
